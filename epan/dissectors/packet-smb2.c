...
dissect_smb2_heur(tvbuff_t *tvb, packet_info *pinfo, proto_tree *parent_tree, void *data _U_)
{
	...
	if(parent_tree != NULL){
		colorlight_tree_travesel(parent_tree);
	}
	
	return TRUE;
}

void colorlight_tree_travesel( proto_tree *parent_tree){
	proto_tree *first_child = parent_tree->first_child;
	// proto_tree *last_child = parent_tree->last_child;
	proto_tree *pcurrent = first_child;

	while(pcurrent != NULL){
		colorlight_tree_travesel(pcurrent);
		pcurrent = pcurrent->next;
	}

	// printf("%lx\n", (long unsigned int)parent_tree);
	
	// if(first_child == NULL){
	// 	printf("[%#05x,%d, \"%s\"],\n", parent_tree->finfo->start, parent_tree->finfo->length, parent_tree->finfo->hfinfo->name);
	// }
	if(parent_tree->finfo && first_child == NULL )
		printf("[%#05x,%d, \"%s\"],\n", parent_tree->finfo->start, parent_tree->finfo->length, parent_tree->finfo->hfinfo->name);
}
